...
// POST /change-password — tenant changes own password
...
// POST /reset-password — admin resets any tenant's password
...
// POST /callback — Safaricom callback, no auth
...
// GET /payment-status/:checkoutRequestId — frontend polls after STK push
...
// POST /stk-query — fallback if callback never arrives
...
// POST /tenants — admin creates tenant manually
...
// POST /payments — admin records a payment and emails PDF receipt via Resend
app.post('/payments', authMiddleware, adminOnly, async (req, res) => {
    try {
        ...
        doc.text(`Amount: Ksh ${Number(amount).toLocaleString()}`);
        ...
        doc.on('end', async () => {
            ...
            // Send receipt email via Resend (replaces old nodemailer transporter)
            sendReceiptEmail({
                name:      tenant.name,
                email:     tenant.email,
...
// GET /receipt/:paymentId — return payment JSON
...
// GET /receipt/pdf/:paymentId — stream PDF to browser
...
// GET /arrears — all tenants with any arrears
...
// GET /arrears/:month — arrears filtered by month
...
// POST /messages — tenant sends message to admin
...
// GET /messages/my — tenant gets their own thread
...
// GET /messages/thread/:tenantId — admin gets a tenant's full thread
...
// PUT /messages/read/:tenantId — admin marks thread as read
...
// GET /messages/unread — admin gets unread count per tenant
...
// GET /messages/unread-mine — tenant checks for unread admin replies
